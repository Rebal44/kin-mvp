...
router.post('/telegram', async (req, res) => {
    ...
    if (text.startsWith('/start')) {
        ...
        if (token) {
            // Handle linking
            try {
                const linkToken = await db_1.prisma.$queryRaw `
        ...
        `;
            ...
            await sendTelegramMessage(chatId, `👋 <b>Welcome to KIN!</b>\n\nSign up at our web app and connect your Telegram to get started.`);
        }
        return;
    }
    // Check if user is linked
    try {
        const link = await db_1.prisma.telegramLink.findUnique({
            where: { telegramUserId },
            include: { user: true }
        });
        if (!link) {
            await sendTelegramMessage(chatId, '❌ Not connected. Please sign up at our web app and connect your Telegram first.');
            return;
        }
        // Check credits
        if (link.user.credits < 5) {
            await sendTelegramMessage(chatId, '❌ Insufficient credits. Please upgrade your plan.');
            return;
        }
        // Create task
        ...
        await sendTelegramMessage(chatId, `✅ <b>Task created:</b> #${task.id.slice(0, 8)}\n\nI'll work on this and reply when done!`);
    }
    catch (error) {
